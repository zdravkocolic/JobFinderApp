...
public final class Utils {
    ...
    public static String errorMessageHandler(Call call, Throwable t) {
        if (t instanceof SocketTimeoutException) { return "Connection timeout, Please try again!"; }
        else if (t instanceof HttpException) {
            ResponseBody body = ((HttpException) t).response().errorBody();
            try { return body.string(); } catch (IOException e) { e.printStackTrace(); }
        }
        else if (t instanceof IOException) { return "Request timeout, Please try again!"; }
        else
        { if(call.isCanceled())
            {
